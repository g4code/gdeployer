...
Git.prototype = {
    ...
    notGitRepo: function()
    {
        evento.trigger("INFORMER_ERROR", this.request.repository+" is not a git repo.");
    },
    ...
    onCheckout: function()
    {
        evento.trigger("INFORMER_LOADING", "Running a git pull on "+this.request.branchName+" branch");
    ...
    onLatestTag: function(latestTag)
    {
        ...
        evento.trigger("EVENT_GIT");
    },

    onPull: function()
    {
        evento.trigger("INFORMER_LOADING", "Checking out to branch "+this.request.branchName);
    ...
    {
        evento.trigger("INFORMER_LOADING", "Running a git pull");
        var pull = new GitPull();
        pull.setRepoPath(this.request.repository)
            .exec()
    ...
    start: function()
    {
        evento.trigger("INFORMER_LOADING", "Checking if repository is git");

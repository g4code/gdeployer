...
Config.prototype = {
    ...
    addToRequest: function()
    {
        ...
        this.request.timestamp       = moment().format("X")
        this.request.datetime        = moment().format("YYYY-MM-DD hh:mm:ss a")
        ...
        this.request.changeFileOwner = this.getChangeFileOwner()
        this.request.log             = this.getLog()
    ...
    getChangeFileOwner: function()
    {
        return _.isUndefined(this.config.changeFileOwner) ?
            null :
            this.config.changeFileOwner;
    },
    ...
    hasLogPath: function() {
        ...
        return typeof this.config.logPath !== 'undefined'
    },

    getLog: function() {

        return {
            logPath: this.hasLogPath() ? this.config.logPath : null,
            dirPath: this.hasLogPath() ? [this.config.logPath, 'gdeployer'].join('/') : null,
            jsonPath: this.hasLogPath() ? [this.config.logPath, 'gdeployer', 'log.json'].join('/') : null,
            htmlPath: this.hasLogPath() ? [this.config.logPath, 'gdeployer', 'log.html'].join('/') : null
        }
    },
